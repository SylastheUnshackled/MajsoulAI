...
# 万筒索(1~9 + m/p/s)，红宝牌用数字5+字母+r；东南西北白发中(E/S/W/N/P/F/C)
all_tiles = set(
    # 普通数牌 1-9
    [str(i)+j for j in ('m', 'p', 's') for i in range(1, 10)] +
    # 红宝牌（5mr/5pr/5sr）
    ['5'+j+'r' for j in ('m', 'p', 's')] +
    # 字牌 (E:东, S:南, W:西, N:北, P:白, F:发, C:中)
    ['E', 'S', 'W', 'N', 'P', 'F', 'C']
)

def majsoul_to_mjai(tile: str) -> str:
    """将 Majsoul 格式的牌转换为 mjai 格式"""
    if tile.endswith('z'):
        # 字牌转换
        number = int(tile[0])
        return {
    ...
    def parse(self, liqi_dict):
        ...
        if method == '.lq.FastTest.authGame':
            ...
            if 'name' in liqi_dict['data']:
                ...
                # 检查 step 值
                current_step = liqi_dict['data'].get('step', 0)
                if hasattr(self, '_last_step') and current_step <= self._last_step:
                    print(f"跳过旧消息: {action_name} step={current_step} (last_step={self._last_step})")
                    return
                self._last_step = current_step
                
                if action_name in ('ActionDiscardTile', 'ActionDealTile', 'ActionAnGangAddGang'):
                    # 单独判断数据里是否有新的明宝牌
                    doras = [majsoul_to_mjai(d) for d in data.get('doras', [])]
                ...
                if action_name in self.no_effect_action:
                    ...
                    tiles = [majsoul_to_mjai(t) for t in data['tiles']]
                    scores = data['scores']
                    leftTileCount = data.get('leftTileCount', 0)
                    assert(len(data['doras']) == 1)
                    self.doras = doras = [majsoul_to_mjai(d) for d in data['doras']]
                    ...
                    tile = majsoul_to_mjai(data['tile'])
                    ...
                    if len(info) > 1:
                        ...
                        pass
                    info = info[0]
                    hand = [majsoul_to_mjai(t) for t in info['hand']]
                    huTile = majsoul_to_mjai(info['huTile'])
                    seat = info.get('seat', 0)
                    zimo = info.get('zimo', False)
                    liqi = info.get('liqi', False)
                    doras = [majsoul_to_mjai(d) for d in info['doras']]
                    liDoras = [majsoul_to_mjai(d) for d in info.get('liDoras', [])]
                    fan = info['count']
                    fu = info['fu']
                    ...
                    tingpai = [players[i].get('tingpai', False)
                               for i in range(4)]
                    hands = [[majsoul_to_mjai(t) for t in players[i]['hand']] if tingpai[i] else []
                             for i in range(4)]
                    scores = data['scores']
                    assert(len(scores) == 1)
                    scores = scores[0]
                    ...
                    seat = data.get('seat', 0)
                    # 处理单张牌或牌组
                    if isinstance(data['tiles'], str):
                        tiles = majsoul_to_mjai(data['tiles'])
                    else:
                        tiles = [majsoul_to_mjai(t) for t in data['tiles']]
